...
def run_online_with_snapshots(
    data: list,
    snapshot_steps=None,
) -> tuple:
    """Run the online perceptron and save (w, b) snapshots at chosen steps.

    Parameters
    ----------
    data           : list of (x, y) pairs (pre-collected via collect_stream_data)
    snapshot_steps : sorted list of example indices at which to save weights.
                     Defaults to five representative moments spread across
                     training, always including t = 0 and the very last step.

    Returns
    -------
    w, b            : final weight vector and bias
    total_mistakes  : total number of mistakes during training
    mistake_history : cumulative-mistake list (one entry per example seen)
    snapshots       : list of (step_index, w_copy, b_copy) tuples
    """
    n = len(data)
    if snapshot_steps is None:
        snapshot_steps = sorted({
            0,
            max(1, n // 20),   # very early
            max(1, n // 5),    # early
            max(1, n // 2),    # halfway
            n - 1,             # near the end
        })
    ...
    for t, (x, y) in enumerate(data):
        if t in snap_set:
            snapshots.append((t, w.copy(), float(b)))
        w, b, m = update_perceptron(w, b, x, y)
        total_mistakes += m
        mistake_history.append(total_mistakes)

    # Always capture the very final trained state
    snapshots.append((n, w.copy(), float(b)))
    return w, b, total_mistakes, mistake_history, snapshots
